...
static fixedp<> _bone_armour_bonus();
...
int player_shield_class()
{
    ...
    shield += _bone_armour_bonus().to_scaled() * 2;
 ...
 *
 * ((power / 100) + 0.5) * (corpse factor).
 * The corpse factor is sqrt(9 + # corpses).
 *
 * @return          The AC/SH bonus.
 */
static fixedp<> _bone_armour_bonus()
{
    ...
    return fixedp<>(you.attribute[ATTR_BONE_ARMOUR]) * (50 + power) / 100;
}
